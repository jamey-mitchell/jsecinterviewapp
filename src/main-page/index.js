...
function App() {
  ...
  const [allSkills, setAllSkills] = useState([
    { name: "Loading...", description: "be patient please" },
  ]);
  ...
  useEffect(() => {
    ...
    const fetchSkills = async () => {
      ...
      const response = await fetch(
        "https://jsecinterviewwebapiexampleapi.azure-api.net/skills"
      );
      const skills = await response.json();
      setAllSkills(skills); //shouldnt this an empty array so it only runs once???
    };
